...
- **Accepted derivations:** Derive a folder class from a readable message class, copy the readable half of a sender name/address pair to the missing half, substitute a valid delivery time for missing creation/modification time, and derive an absent associated-message display name from its readable subject. These values retain usable source facts rather than inventing unrelated metadata.
...
### EMP-15
- **Status:** Accepted recovery policy for source-derived recipient values and permanent bounded provenance accounting; generated attachment fallback values remain pending owner decision
...
- **Accepted derivations:** Copy a readable recipient display name or address to its missing counterpart. This retains the available source identity while preserving the recipient role.
