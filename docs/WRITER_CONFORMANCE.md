...
- **Implementation:** `table_context*`, `write_external_table_value`, `push_heap_allocation`, compact-to-external hierarchy/contents/associated fallback, `schema_columns`, `write_table_value`, `mark_column`
...
- **Implementation:** `collect_named_identities*`, `named_property_map`, `build_named_property_context`, `externalize_large_properties`, `validate_named_map`
- **Evidence:** named ordering, custom GUID, empty map, embedded map, and scalable NAMEID property-context tests; prior ScanPST fidelity candidates; the owner reports clean ScanPST analysis of all four 0.6.0 direct 50 GB qualification parts containing the 562-identity catalog
