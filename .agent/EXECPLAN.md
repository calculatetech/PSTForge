...
- Observation: Once Debian tests passed, the checkout's final `git diff
  --check` returned Git's usage status 129 under the root-owned container step.
  The gate now marks its known checkout explicitly with a command-local
  `safe.directory` value and publishes a bounded `diff-check.log` alongside the
  bounded public test diagnostic if this platform integration fails again.
  Evidence: GitHub Actions run 29796156056, Debian job 88527828807.
