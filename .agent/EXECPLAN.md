...
- [ ] Milestone 0.5.0: Operational UX and Debian Packaging.
  ...
  - [x] Checkpoint 4: replace the stale README with current features,
    ...
    package name. No ScanPST or Outlook check is required unless this milestone
...
- [x] Milestone 0.6.0: Interoperability Release Candidate.
  ...
  - [x] (2026-07-21) Ran the bounded five-minute `pst_reader` fuzz target on
    pinned nightly Rust in hosted scheduled run `29870842973`; the generated,
    ephemeral corpus passed in 6m21s without retaining private input.
  - [x] (2026-07-21) Completed the local and GitHub acceptance matrix, bounded
    fuzzing, fault coverage, and security/license/privacy/data-loss review.
    Final fast, canonical full, and release gates passed; required Ubuntu,
    Debian, and CodeQL checks passed both on the pull request and after merge.
  - [x] (2026-07-21) Completed ScanPST-first interoperability acceptance. The
    owner reported clean ScanPST results for all four direct 50 GB parts and
    successful Outlook acceptance. Those checks are the PSTForge boundary;
    MailPlus compatibility is assumed and no MailPlus resource or import gate
    is required. Any later MailPlus-only failure belongs in a Synology ticket.
...
- Decision: Clean ScanPST analysis followed by successful Outlook opening and
  ...
  the intended deployment consumer, but its import service is not a required
  external test resource or a release gate. After ScanPST and Outlook pass,
  MailPlus compatibility is assumed; a failure confined to MailPlus is raised
  with Synology and does not create PSTForge remediation work.
  Rationale: ScanPST supplies Microsoft's structural integrity check and
  Outlook supplies the consuming implementation check. Requiring a separately
  hosted MailPlus environment adds an external operational dependency without
  improving PST-format evidence after both Microsoft checks succeed.
  Date/Author: 2026-07-21 / human owner direction.
...
- Decision: Direct mode performs one source-content traversal and one
  ...
  mandatory CI, adversarial-review, ScanPST, and Outlook acceptance layer, not
  a production transformation stage. Restartable mode retains
...
Version 0.6.0 is accepted and merged through protected pull request #2. Fresh
output paths self-create without weakening path safety; the damaged 50 GB
source completes in both direct and restartable modes with exact semantic
accounting; repaired-reference comparison rules out readable loss; and scale
evidence identifies writer/SQLite lifecycle rather than libpff traversal as
the end-to-end bottleneck. All four direct parts passed ScanPST and Outlook.
That pair is now the release interoperability boundary, so no MailPlus-hosting
resource or import rehearsal remains before 1.0.
...
structural checker, `pffinfo`, and `readpst`, then require clean ScanPST and
successful Outlook inspection before promotion. MailPlus import is optional
operator evidence and does not control promotion.
...
complete, open each original part independently in Outlook and compare folder
and message counts, unread/deleted placement, sampled bodies, and attachments.
An optional MailPlus import does not alter acceptance.
...
and Outlook. The first cold attempt was stopped after 57:47 with only
...
milestone, so human ScanPST and Outlook testing is not a gate.
...
license, privacy, and data-loss reviews. Require clean ScanPST analysis of
representative parts, then open and inspect the same parts in supported
Outlook. These checks establish the PSTForge interoperability boundary. Do not
require a MailPlus test environment; MailPlus-specific failures after these
checks are Synology support issues.
...
source and validate all parts with ScanPST and Outlook. Confirm source
...
ScanPST/Outlook evidence checks, reproducibility, and release documentation.
...
Human interoperability acceptance runs ScanPST first and then opens the same
unrepaired part in Outlook, expands folders, samples content and attachments,
and records only bounded conclusions and artifact hashes. MailPlus import is
optional operator evidence, not a PSTForge gate; a failure confined to
MailPlus after both Microsoft checks pass is referred to Synology support.
