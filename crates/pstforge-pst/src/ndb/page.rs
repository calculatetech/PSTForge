...
use crate::{
    AnsiPstFile, PstFile, PstReader, UnicodePstFile, block_sig::compute_sig, crc::compute_crc,
};
    ...
    fn find_free_bits(&self, max_size: u16) -> Range<u16> {
        ...
        for &page in self.map_bits() {
            if page == 0 {
                ...
                assert!(
                    leading_zero + trailing_zero < 8,
                    "leading_zero: {leading_zero}, trailing_zero: {trailing_zero} for page: 0x{page:02X}"
                );
...
impl UnicodeBlockBTreeEntry {
    pub fn new(block: UnicodeBlockRef, size: u16) -> Self {
        Self::new_with_ref_count(block, size, 1)
    }

    pub fn new_with_ref_count(block: UnicodeBlockRef, size: u16, ref_count: u16) -> Self {
        Self {
            block,
            size,
            ref_count,
