...
const JOB_SCHEMA_VERSION: i64 = 10;
...
mod tests {
    ...
    #[test]
    fn resume_rejects_schema_nine_without_appointment_classification()
    -> Result<(), Box<dyn std::error::Error>> {
        let directory = tempdir()?;
        let job = directory.path().join("job");
        let sink = DurableCatalogSink::create(&job)?;
        ...
        connection.execute(
            "UPDATE job_metadata SET value = '9' WHERE key = 'schema_version'",
            [],
        )?;
        drop(connection);
        assert!(matches!(
